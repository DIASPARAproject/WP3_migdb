...
![](images/clipboard-3991683683.png)
:::
...
::::: columns
::: {.column width="50%"}
GFCM - RDBFIS This information could be used in the upcoming RDBFIS database as well
:::

::: {.column width="50%"}
DCF data MAKE CHANGES HERE!
:::
:::::
...
# Principles {.scrollable .incremental}
:::{.incremental}
* Dictionaries
  * Follow ICES vocab, create when not existing (search for all vocab)
  * Allow for the hierarchichal spatial scale to be integrated in queries, the structure of the db should allow for this.
     Separate the input of different WGs in different schema but the same db (postgres).
...
* Model
  * Separate parameters for the model and aggregated data built at the regional / national/ emu level.
  * Model parameters should be described in a separate metadata table (Unit, definition ...) like WGNAS.
*  Keep change minimal, don't add work to working group
...
# {background-image=images/title_slide_thank_you.png}
