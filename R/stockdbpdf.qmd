...
format: 
  pdf:
    ...
    include-in-header:  
      - text: |
            \usepackage{tikz}
    include-before-body: stockdbpdf_header.tex
    toc: true
    toc-depth: 3
    toc-title: Contents
    number-sections: true
...
This documents creates the diaspara database, first creating referential table for the
...
All along this document, the database will be named `DiadromousDB`. The
...
-   Code ELE, ANG, TRS are capitalized, as are the working group names
...
::: {.callout-note appearance="simple"}
<h3>An eel is not an elephant </h3>
To avoid generations of researchers looking at the columns and wondering
the meaning of `ELE` (the current code for eel in ICES) and wondering 
about the secret link between eels and elephants we have used `ANG`.
 This column with species will be everywhere.
 For the others we've taken as close as possible to the latin
name. Finally despite using TRT initially following Iain advice
we have also used both current code for sea trout `TRS` and `TRT` as data
have already been entered in RDBES with the `TRS` code.
:::
...
value = tr_species_spe_temp, overwrite = TRUE)
...
tr_icworkinggroup_wkg <-
 dbGetQuery(con_diaspara, "SELECT * FROM ref.tr_species_spe")
...
#| tbl-cap: Working groups table in the diaspara DB
tr_icworkinggroup_wkg <- 
dbGetQuery(con_diaspara, "SELECT * FROM ref.tr_icworkinggroup_wkg")
knitr::kable(tr_icworkinggroup_wkg) |>
 kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
```
...
query <- sprintf("INSERT INTO ref.tr_units_uni (uni_code,uni_description, 
uni_icesvalue, uni_icestablesource,uni_icesguid)
...
::: answerbox-body
https://github.com/ices-eg/DIG/issues/737
:::
::::::
...
dbGetQuery(con_diaspara, "SELECT * FROM ref.tr_objecttype_oty;")|>
 knitr::kable() |> 
 kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
...
dbGetQuery(con_diaspara, "SELECT * FROM ref.tr_nimble_nim;")|> 
knitr::kable() |> 
kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
...
dbGetQuery(con_diaspara, "SELECT * FROM ref.tr_version_ver;")|>
 knitr::kable() |>
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
...
dbGetQuery(con_diaspara, "SELECT * FROM ref.tr_metric_mtr;")|> knitr::kable() |> 
kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
...
dbGetQuery(con_diaspara, "SELECT * FROM ref.tr_category_cat;")|>
 knitr::kable() |>
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
...
laying out the foundations. Not assessment unit correpsond to 'Unit' level in the 
for the Baltic.
...
dbExecute(con_diaspara_admin,
          "INSERT INTO ref.tr_habitatlevel_lev VALUES( 
  'Stock',
  'Correspond to stock units for which advices are provided in ICES,
   this can be the level of the panpopulation,
...
dbExecute(con_diaspara_admin,"UPDATE ref.tr_habitatlevel_lev
	SET lev_description='Corresponds to an assessment unit in the Baltic sea,
   and area for WGNAS, and EMU for WGEEL.'
...
![First concepts of the hierarchy ](fig-area_hierarchy.svg "A schema showing the possible hierarchy from stock level to river"){#fig-area_hierarchy}
...
'NC',	
'Not collected',	
'Activity / habitat exists but data are not collected by authorities 
(for example where a fishery exists but the catch data are not collected at
 the relevant level or at all).';")
dbExecute(con_diaspara_admin,
          "INSERT INTO ref.tr_missvalueqal_mis 
SELECT
'NP',	
...
(for example where catch data are absent as there is no fishery or
 where a habitat type does not exist in a stock unit).';")
...
kable(TS_MATURITY, caption = "TS_MATURITY") |>
 kable_styling(bootstrap_options = c("striped", "hover", "condensed")) 
kable(TS_DevStage,
 caption = "Devstage scale, this seems to be about shrimp 
 (berried seems to an egg which looks at you with eyes in shrimps....)") |>
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) 
```
...
dbExecute(con_diaspara_admin,
"DELETE FROM ref.tr_lifestage_lfs WHERE lfs_spe_code ='ANG';")
dbExecute(con_diaspara_admin,
 "INSERT INTO ref.tr_lifestage_lfs (lfs_code,lfs_name,lfs_description, lfs_spe_code)
...
dbExecute(con_diaspara_admin, 
"INSERT INTO ref.tr_lifestage_lfs 
    (lfs_code, lfs_name, lfs_spe_code, lfs_description, lfs_icesvalue, 
    lfs_icesguid, lfs_icestablesource)
    SELECT 
    lfs_code, 
    lfs_name, 
...
dbExecute(con_diaspara_admin, 
"INSERT INTO ref.tr_quality_qal (qal_code, qal_description, qal_definition, qal_kept)
SELECT qal_id, qal_level, qal_text, qal_kept FROM refwgeel.tr_quality_qal ;")
# 13
# changing one definition linked to wgeel
dbExecute(con_diaspara_admin, 
"UPDATE ref.tr_quality_qal set qal_definition = 'the data passed the quality checks and is considered as good quality' WHERE qal_code = 1")
# This one only causes problems.... Remove.
dbExecute(con_diaspara_admin, 
"DELETE FROM ref.tr_quality_qal WHERE qal_code = 0")
...
kable(TS_SEXCO, caption = "TS_SEXCO") |> 
kable_styling(bootstrap_options = c("striped", "hover", "condensed")) 
```
...
## Time period (ref.tr_timeperiod_tip) WGBAST
...
## Data source (ref.tr_datasource_dts) WGBAST {#sec-datasource}
...
## Data basis ref.tr_databasis_dtb (WGBAST)
...
The estimationmethod are set in each working group table, they are inherited.
see refbast.tr_estimationmethod_esm
:::
...
dbGetQuery(con_diaspara, "SELECT * FROM dat.t_metadata_met limit 50;") |> 
knitr::kable()|>
kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
...
dbGetQuery(con_diaspara, "SELECT * FROM datnas.t_metadata_met limit 10;")|>
 knitr::kable() |> 
 kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
...
and then for trutta. These are no duplicates as the primary key is set on both TRS
and SAL. Later on, there should be variables only with SAL (at the second step
...
```{r datbast.t_stock_sto_young_fish}
...
tp_type has 89 lines with COMM =>
Assigned to Year to be checked
:::
:::::
