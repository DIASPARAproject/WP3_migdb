...
[metric database](https://diaspara.bordeaux-aquitaine.inrae.fr/deliverables/wp3/p11metric/rep11.html)
...
Aside the main database report, which describes all the vocabularies used in this document, we have to build a separate database for metrics. In the project these correspond to Life History Traits (LHT), but additionally, the data should also correspond to the time series and the sampling db developed by WGEEL. The two latter data structures (series and sampling) are very similar, and they both hold very similar group metrics and individual metrics.
...
* The third objective is to import data from Diaspara WP2, the excel sheets have been created in february 2025 and will already (in March) require some adaptation as the database evolves, for instance the referential of stages is no longer in line with the templates.
...
To include the station, we need two tables: the first one is the Station dictionary
...
This one was formerly called metric, but metric `ref.tr_statistic_sta` already exist and is better
...
"tra_indivorgroup"=stringi::stri_trans_totitle(tra$mty_group),
...
tranum[tranum$tra_indivorgroup=="Both" & grepl("proportion",tranum$tra_code),
c("tra_indivorgroup")] <- "Group"
...
trn_maxvalue
 ...
 tra_wkg_code,tra_spe_code,tra_indivorgroup,tra_qualitativeornumeric,
...
<summary>SQL code to create table `datbast.t_series_ser`</summary>
...
```{r refbast.t_series_ser_import_trutta}
#| echo: TRUE
#| eval: FALSE
#| warning: FALSE
#| message: FALSE
#| code-fold: TRUE
#| code-summary: Code to import to to refbast.tr_series_ser
