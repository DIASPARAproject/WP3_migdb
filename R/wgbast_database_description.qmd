...
#| warning: FALSE
#| message: FALSE
#| tbl-cap: Habitat type in column `fishery` in wgbast catch database. Rows correspond to the values in sub_div, columns to habitat type
...
The values for habitat are : `r unique(catchdb$fishery)` there are `r sum(is.na(catchdb$fishery))` NA values in the database.
...
::::
::::{.questionbox-body}
The name of the column `fishery`is not very explicit, would the use of `habitat_type` be more in line with data ?
::::
:::
...
:::{.callout-note appearance="simple"}
## NOTE
There are `r sum(is.na(catchdb$f_type))` NA values for `f_type`.
:::
...
::::
::::{.questionbox-body}
39 lines with `BROOD` do you know to what these correspond ?
::::
:::
...
#| warning: FALSE
#| message: FALSE
#| tbl-cap: Source of data `n_type` in the WGNAS fisheries database.
#| tbl-subcap: 
