...
ver_code = paste0("SAL_",2020:2024),
ver_year = 2020:2024,
ver_spe_code = "SAL",
ver_datacalldoi=c(NA,NA,NA,NA,"https://doi.org/10.17895/ices.pub.25071005.v3"), 
ver_stockkeylabel =c("ele.2737.nea","sal.27.22–31","sal.neac.all"), # sugested by Hilaire.
# TODO FIND other DOI (mail sent to ICES)
ver_version=c(1,1,1,1,1), # TODO WGNAS check that there is just one version
...
overwrite = TRUE)
dbExecute(con_diaspara_admin, "INSERT INTO ref.tr_version_ver SELECT * FROM temp_tr_version_ver;") # 5
...
#| message: FALSE
#| tbl-cap: category of parameters
dbGetQuery(con_diaspara, "SELECT * FROM ref.tr_deestination_dest;")%>% knitr::kable() %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
...
"CREATE TABLE ref.tr_area_are (
  ...
  CONSTRAINT fk_area_wkg_code FOREIGN KEY (are_lev_code) REFERENCES
...
res <- dbGetQuery(con_diaspara, "SELECT * FROM refnas.tr_metadata_met;")
clipr::write_clip(colnames(res))

data.frame(
met_var= dbGetQuery(con_salmoglob, "SELECT var_mod FROM metadata"),
met_spe_code = 'NAS',
met_wkg_code = 'WGNAS',
met_ver_code = TODO
met_oty_code
...
<summary>Creating the referential for WGNAS , work in progress ...
</summary>
