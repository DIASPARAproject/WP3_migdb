...
``` {.sql include="../SQL/2_ref_tr_objectype_oty.sql"}
```
...
``` {.sql include="../SQL/2_dat.t_metadata_met.sql"}
```
...
dbGetQuery(con_diaspara, "SELECT are_id,
   ...
   are_lev_code,
   are_wkg_code,
   are_ismarine FROM ref.tr_area_are;")%>%
  knitr::kable() %>%
...
``` {.sql include="../SQL/4_datnas_t_metadata_met.sql"}
```
...
dbGetQuery(con_diaspara, "SELECT * FROM refnas.t_metadata_met limit 10;")%>% knitr::kable() %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
...
``` {.sql include="../SQL/4_dat.t_stock_sto.sql"}
```
...
``` {.sql include="../SQL/4_tg_additional_add.sql"}
```
...
## structure of the table refnas.t_stock_sto
