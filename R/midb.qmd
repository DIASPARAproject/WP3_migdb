...
Table tr_quality_qal used by the wgeel {# tbl-tr_quality_qalwgeel}
...
The code for creating `tr_age_age` (Table @tb-tr_age_age) is shown below.
...
```
## Fisheries tr_fishery_fiy


The code for creating `tr_fishery_fiy` (Table @tb-tr_age_age) is shown below.

<details>

<summary>SQL code to create table `tr_fishery_fiy`</summary>

``` {.sql include="../SQL/2_ref_tr_fishery_fiy.sql"}
```

</details>

```{r}
#| label: tbl-tr_fishery_fiy
#| echo: TRUE
#| eval: TRUE
#| warning: FALSE
#| message: FALSE
#| code-fold: TRUE
#| tbl-cap: Table of age
dbGetQuery(con_diaspara, "SELECT * FROM ref.tr_fishery_fiy;") %>% 
knitr::kable() %>% 
kable_styling(bootstrap_options = c("striped", "hover", "condensed"))

```

## Additional column tg_additional_add
...
We need to store those in an additional table.
...
dbWriteTable(con_diaspara,  "temp_stock", stock)
dbExecute(con_diaspara,"INSERT INTO datnas.t_stock_sto
(sto_id, sto_met_var, sto_year, sto_spe_code, sto_value, sto_are_code, sto_cou_code, sto_lfs_code, sto_hty_code, sto_fia_code, sto_qal_code, sto_qal_comment, sto_comment, sto_datelastupdate, sto_mis_code, sto_dta_code, sto_wkg_code)
SELECT 
nextval('dat.t_stock_sto_sto_id_seq'::regclass) 
, var_mod
'', 0, 'SAL'::character varying, 0, '', '', '', '', '', 0, '', '', '', '', 'Public'::text, 'WGNAS'::text
FROM refsamoglob.database
;
```
