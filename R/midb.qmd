...
::: {.callout}
<h3> QUESTION ICES: missing values for units, what do we do ? <\h3>
:::
...
This will be continued in t_stock_tso

```{r refeel.tr_area_are}
#| echo: TRUE
#| eval: FALSE
#| warning: FALSE
#| message: FALSE
...
*    type not a referential, used for legacy in WGNAS 
...
``` {.sql include="../SQL/6_dateel.t_stock_sto.sql"}
```


<summary>SQL code to insert values in table `dateel.t_stock_sto` </summary>

``` {.sql include="../SQL/6_dateel.t_stock_sto_insert.sql"}
```
