...
``` {.sql include="../SQL/6_dateel_t_stock_sto.sql"}
```


<summary>SQL code to insert values in table `dateel.t_stock_sto` </summary>

``` {.sql include="../SQL/6_dateel_t_stock_sto_insert.sql"}
```
...
* TODO CHECK WITH MARIA about the unit
...
res  <-  dbGetQuery(con_diaspara, "SELECT * FROM datbast.t_metadata_met;")
clipr::write_clip(colnames(res))
head(t_metadata_met)
...
t_metadata_met  <- data.frame(
    ...
    met_cat_code = case_when(
       ...
       met_type == "BMS" ~ "Discarded",
       .default = NA),
met_type = met_type, # historical code in the db corresponds to f_type
met_definition = "TODO",
met_deprecated = NA 
# not integrating any of the deprecated data
)
...
species met_spe_code sto_spe_code
country              sto_cou_code
year                 sto_year
time_period          => decide either different tables or a different structure of t_stock_sto
tp_type
sub_div              sto_are_code (decide what to do with continental landings)
f_type               WHy ARE SOME TYPES LEFT BLANK (SE FI 200 300)
gear                 
effort 
weight   
w_type     
...
w_ci      
n_ci       
gear2          Why is this different from gear ?
subdiv_ic  
hyr        
