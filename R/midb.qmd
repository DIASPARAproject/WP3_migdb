...
<summary>Creating the referential for WGBAST</summary>

``` {.sql include="../SQL/3_refbast_tr_version_ver.sql"}
```
...
t_metadata_met_SAL <- t_metadata_met_TRT
t_metadata_met_SAL$met_spe_code<- "SAL"


t_metadata_met <- bind_rows(t_metadata_met_TRT,t_metadata_met_SAL)
...
SELECT met_var, met_spe_code, met_wkg_code, met_ver_code, met_oty_code, met_nim_code, met_dim::integer[], met_dimname::text[], met_modelstage, met_type, met_location, met_fishery, met_mtr_code, met_des_code, met_uni_code, met_cat_code, met_definition, met_deprecated FROM temp_wgbast_t_metadata_met") #42
...
df_all_N <- df_all |>
  filter(!is.na(numb)) |>
  mutate(sto_met_var = paste0(tp_type, "_N"))
