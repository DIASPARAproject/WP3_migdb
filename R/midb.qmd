...
In the report the stages are defined as following :
...
: SMSF (WKMATCH 2012 maturity scale revised). Source:
\[\@ices_wbbiop_2024\].
...
-   the substage Bb identifies a developing and functionally mature
    ...
    following codes are proposed.

\@ (Table @tbl-tr_maturity_mat).
...
mostly similar except that `C` (coastal) in WGEEL is `MC` in WLTYP and
freshwater is `F` instead of `FW`. WGBAST separates Marine Open `O`
(instead of `MO`), Marine coastal `C` (instead of `MC`) and rivers `R`
(instead of `FW)`. In the report it is said that `S` sea is used when it
is not possible to distinguish between coastal and marine open, but the
code is not in the database (If I'm not wrong see [WGBAST database
description - catch
...
#| code-fold: TRUE
#| message: FALSE
#| tbl-cap: ICES vocabularies for station type
...
Download shapefiles from NAFO =\> create table following ICES_Areas for
GUID FAO major fishing area (27, 21, 37, 34, 31) 27 Atlantic, Northeast
21 Atlantic, Northwest 37 Mediterranean and Black Sea 34 Atlantic,
Eastern Central 31 Atlantic, Western Central subarea division
subdivision
...
are different, however, more than the previous ones. The values for
