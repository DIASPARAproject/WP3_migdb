...
Thinking later about this, I chose to get rid of `ELE` and select `ANG`. This column with species will be everywhere. We want to avoid generations of researchers to avoid what `ELE` is (apart from elephants). For the others I've taken as close as possible to the latin name.
...
IS 'Code of the variable, primary key on both met_spe_code and met_var_code';
COMMENT ON COLUMN ref.tr_metadata_met.met_spe_code 
IS 'Species, ANG, SAL, TRT ... primary key on both met_spe_code and met_var_code';
