...
``` {.sql include="../SQL/6_dateel.t_metadata_met.sql"}
```
...
t_metadata_met <-
  data.frame(
    ...
    met_nim_code =  case_when(    
      typ$typ_id %in% c(4,6,8,9,),
      "Const_nimble" == metadata$nimble ~ "Parameter constant",
      "Output" == metadata$nimble ~ "Output",
      "other" == metadata$nimble ~ "Other",
      .default = NA), # https://diaspara.bordeaux-aquitaine.inrae.fr/deliverables/wp3/p7/midb.html#type-of-parm-data-tr_nimble_nim
    met_dim = paste0(
      "{", metadata$dim1, ",",
      replace_na(metadata$dim2, 0), ",",
      replace_na(metadata$dim3, 0), "}"
