...
1) there is a problem in the "order or the dimensions which need to be aligned. For instance a column can hold year, or age. This is not good. The description could be used within a type [array](https://www.postgresql.org/docs/current/arrays.html)  more conveniently,
2) we just want to use vocabs as close to the referentials as possible
...
#| warning: FALSE
#| message: FALSE
#| tab-cap: Access to the advice using icesASD
#| 
...
```{sql metadata_and_related_tables}
--| echo: TRUE
--| eval: FALSE
...
## DIASPARA
 ...
 But they are also in the database itself
 We will remove them to reduce complexity and avoid errors.
 The complex will be derived from the spatial structure
:::
...
This table (@ tbl-tr_objectype_oty) is used in metadata 

```{r}
#| label: tbl-tr_objectype_oty
#| echo: TRUE
#| eval: TRUE
#| warning: FALSE
#| message: FALSE
...
In the salmoglob db, this table corresponded to both tables status and nimble 
which most often contained the same information.

```{r}
#| label: tbl-tr_nimble_nim
#| echo: TRUE
#| eval: TRUE
#| warning: FALSE
#| message: FALSE
...
The version contains both reference to the datacall (when data are loaded)
...
::::
::::{.questionbox-body}
I would like to access to this table [datacall](https://data.ices.dk/DataCalls/listDataCalls)? 
...
::: {.callout-note appearance="simple"}
## NOTE TO SELF
Some code will have changed, be sure to update (e.g. change Standard Deviation to SD, before pulling the data
:::


# Metric (tr_metric_mtr)

The metric is the mathematical type of value or parameter stored. Parameters can be
either prior or posterior (estimate).
