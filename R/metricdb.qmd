...
Aside the main database report, which describes all the vocabularies used in this document, we have to build a separate database for metrics. 

<!--HILAIRE should we say they are separate or should we say it is a subpart of the diadromous database, alongside the stock database--> 

In the project, these correspond to Life History Traits (LHT), but additionally, the data should also correspond to the time series, the sampling db developed by WGEEL, counting at traps, electrofishing data for Sea Trouts or Salmon juvenile.

The two latter data structures (series and sampling) are very similar, and they both hold very similar group traits and individual traits 

<!--HILAIRE yes, but abundance indices are rather different and used in stock assessment, either directly or after preprocessing. I don't have any solutoins, but mixing metrics and abundance indices seems a bit weird to me--> 

<!--HILAIRE yes, but abundance indices are rather different and used in stock assessment, either directly or after preprocessing. I don't have any solutoins, but mixing metrics and abundance indices seems a bit weird to me--> .



The first was developed initially to store data about the series used in recruitment data. In practice, it consists of three tables, the `t_series_ser` (Figure @fig-series_diagram_wgeel - top in blue) table contains series id and description, with columns describing the sampling details, the stage used, the method... This is the main identifier of the series which will be used as a reference in all dependent tables. The second `t_dataseries_das` table  (Figure @fig-series_diagram_wgeel - on the right) holds data about annual values in series. These are typically annual counts for recruitment, along with additional effort data. Linked to these are group metric series used to describe the series, mean age of eel, mean size, proportion of glass eel among the yellow eels, proportion of females ... (Figure @fig-series_diagram_wgeel - in orange)
...
The second type of data was developed to hold the data collected for DCF. These can be metrics collected from sampling by the fishermen {>>you mean sampling on landings<<}, data coming from the analysis of electrofishing data, or other experimental sampling that are not reported as series. 

<!--HILAIRE I think here, you are making things confusing: the idea was to store biometry data, i.e. data on characteristics of the fish. When you refer to electrofishing, they can also provide abundance indices, but they do not aim be stored in the othersampling table. Same for landings: othersampling aim to collect biometry data on fish collected for example among landings, but not to store the landings themselves. So, if I should make a distinction, I would rather say we collect fish characteristics data (lht, quality etc.) and time series of population dynamics (landings, time series of abundance, releases...), then in both types, the data can come from specific monitoring sites, from some electrofishing monitoring network, from DCF, we don't care.--> 

<!--Cédric The idea was to explain the two rather close structures that we have in WGEEL, and explain why we wanted to merge them. This was relevant at the beginning when trying to explain what we were developping, I'm not sure it is now. So I keep Hilaire's comment for rephrasing the use and content of the two databases. -->



Currently the two structures for series and sampling are very close, the only difference is that there is no annual number linked to the sampling data, and that they are not linked to a stage in the first table, so the stage is added in the fish table 
<!--HILAIRE, let's just put annual number elsewhere, in the stock db, they are more aimed at informing on the stock trend anyway-->.

<!--CEDRIC, no the group metrics refer to the series. But this is solved below, by our merging the two DB. -->.


The difference in table structure is illustrated below in tables highlighted in yellow (Figure @fig-sampling_diagram_wgeel). 

<!--HILAIRE my comments here are purely cosmetic: it doesn't change at all the structure you propose and the content of the db: just a way to rephrase it. You have a stock db for stuffs that are mostly to inform on population dynamics, and a biometry db that store fish characteristics, mainly biometry but possibly other features? Since all of them are in fact a single db and data sources are documented, no problem -->.
...
* The first objective is to join the two databases to simplify the database development and handling of data.

* The second objective is to use the new referentials created for the migdb database.

* The third objective is to import data from Diaspara WP2, excel sheets have been created in february 2025 to store data for Salmon and will require some adaptation as the database changed during the project.
...
<!--CEDRIC COPY THIS PART INTO THE DICTIONARIES -->

## Station


The metric data need to be related to  sampling stations. These rely on a set of vocabularies (EDMO, PRGOV ...) liseted hereafter.
To include the stations, we need two referential tables : the first one is the Station dictionary and the second is the relation Dictionary (which relates stations together). Both are maintained at ICES.  
...
traqal <- data.frame(
  ...
  "tra_wkg_code" = 'WGEEL',
  "Tra_spe_code" = 'ANG',
  "tra_uni_code" = tra$mty_uni_cotra_indivorgroup"='Individual',
...
The station are loaded into a temporary table that will not be
exactly like ICES's, and will later be replaced by ICES stations. 
...
This table is the same as in wgnas, it is inherited from ref. Currently working with inherited table will allow for more flexibility along the working groups. 
...
Fish traits table aim to store information collected on individual fish. However, for various reasons, similar data are often collected and averaged on batches of individuals. We created a group metric to store those data
...
The following scripts are used to import group metric from the two existing data formats, sampling and series. 
...
dbWriteTable(con_diaspara_admin, "tr_traitnumeric_trn_temp", tranum, overwrite =TRUE)
dbExecute(con_diaspara_admin, "DELETE Ftra_indivorgrouptraitnumeric_trn")
...
dbExecute(con_diaspara_admin, "DELETE FROM refntra_indivorgrouplitative_trq")
dbExecute(con_diaspara_admin, "INSERT INTO refnas.tr_traitqualitative_trq(
tra_id,
tra_codetra_indivorgroupe,
tra_spe_code,
tra_description,
tra_typemetric,
tra_qualitativeornumeric
