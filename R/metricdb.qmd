...
```{r ref_ref_tr_traitnumeric_trn_insert}
#| echo: TRUE
#| eval: FALSE
#| warning: FALSE
#| message: FALSE
#| code-fold: TRUE
#| code-summary: Code to import trait table.
...
"tra_group"=tra$mty_group)
tr_trait_tra_temp$tra_individualname[tr_trait_tra_temp$tra_group=="group"] <- NA
tr_trait_tra_temp$tra_groupname[tr_trait_tra_temp$tra_group=="individual"] <- NA
dbWriteTable(con_diaspara_admin, "tr_trait_tra_temp",tr_trait_tra_temp)
dbExecute(con_diaspara_admin, "INSERT INTO ref.tr_trait_tra 
SELECT 
tra_code,
tra_description,
tra_individualname,
...
meth <- dbGetQuery(con_diaspara, "SELECT * FROM ref.tr_trait_tra;")
knitr::kable(trait) %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
```
## Fish trait measurement method

In individual metrics the metric used is either a measurement,
or a category, or a method. 
In the new database, the metric
...
## Fish trait measurement limits

For each trait we can store a measurement limit, currently this is in the table,
it will probably go to DATSU.

<details>

<summary>SQL code to create tables</summary>

``` {.sql include="../SQL/metric_08_ref_tr_traitlimit_lim.sql"}
```
