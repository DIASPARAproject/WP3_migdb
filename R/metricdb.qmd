...
subtitle: "Trait dataset final report"
...
Aside the main database report, which describes all the vocabularies used in this document, we have to build a separate database for trait data (called metric previously in the wgeel database). 
...
Here we import files from WP2, this section is meant to test the inclusion of salmon data in the database.
...
``` {.sql include="../SQL/trait_57_ref_tr_traitvaluequal_trv.sql"}
```
</details>
...
``` {.sql include="../SQL/trait_59_refnas_tr_traitmethod_trm.sql"}
```
...
``` {.sql include="../SQL/trait_60_datnas_t_indivtrait_int.sql"}
```
