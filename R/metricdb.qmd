...
Aside the main database report, which describes all the vocabularies used in this document, we have to build a separate database for metrics. {>>should we say they are separate or should we say it is a subpart of the diadromous database, alongside the stock database?<<} In the project these correspond to Life History Traits (LHT), but additionally, the data should also correspond to the time series and the sampling db developed by WGEEL {>>what about counting at traps, parr indices at spawning grounds etc? don't they have this kind of stuff in WGBAST?<<}. The two latter data structures (series and sampling) are very similar, and they both hold very similar group metrics and individual metrics {>>yes, but abundance indices are rather different and used in stock assessment, either directly or after preprocessing. I don't have any solutoins, but mixing metrics and abundance indices seems a bit weird to me<<}.
...
* The third objective is to import data from Diaspara WP2, the excel sheets have been created in february 2025 and will already (in March) require some adaptation as the database evolves, for instance the referential of stages is no longer in line with the templates. {>>which templates are you refering too?<<}
