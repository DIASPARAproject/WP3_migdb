...
# Hierarchical structure of the stock DB {#sec-hierarchical}

The database starts with the simplest structure, with a basic schema
corresponding to all data. Then three schema are created, one per
...
To create the units table, we started from the existing WGEEL table, and later added
some elements for WGBAST.
To standarize using ICES codes, we have checked several existing vocabularies.
...
structure](images\SAM_parm_metadata.png){#fig-metadata .lightbox}
...
are specified in table @tbl-objectype. This table is used in metadata, where each parameter is described.
...
## Type of parm / data (tr_typeparm_tpa)

In the salmoglob database (WGNAS), this table (@tbl-typeparm) corresponds to both
...
``` {.sql include="../SQL/2_ref_tr_typeparm_tpa.sql"}
```
...
The vocab described here is deprecated and superseeded by datasource.
...
t_metadata_met_TRS  <- data.frame(
    ...
    met_oty_code = "Single_value", #  https://diaspara.bordeaux-aquitaine.inrae.fr/deliverables/wp3/p7stock/midb.html#object-type-tr_objectype_oty
    met_nim_code =  "Data", # https://diaspara.bordeaux-aquitaine.inrae.fr/deliverables/wp3/p7stock/midb.html#type-of-parm-data-tr_bayestype_bty
    met_dim = paste0(
      "{", 1, ",",
       0, ",",
       0, "}"
...
t_metadata_met_a  <- data.frame(
    ...
    met_nim_code =  ifelse(array$type == "stockastic", "Data", "Output"), #scenarios or stockastic TODO check consistency with WGNAS https://diaspara.bordeaux-aquitaine.inrae.fr/deliverables/wp3/p7stock/midb.html#type-of-parm-data-tr_bayestype_bty
    met_dim = paste0(
      "{", array$dim1, ",",
...
t_metadata_met_s  <- data.frame(
    ...
    met_nim_code =  ifelse(scalar$type == "stochastic", "Data", "Output"), #scenarios or stochastic TODO check consistency with WGNAS https://diaspara.bordeaux-aquitaine.inrae.fr/deliverables/wp3/p7stock/midb.html#type-of-parm-data-tr_bayestype_bty
    met_dim = paste0(
      "{", 1, ",",
       0, ",",
       0, "}"
...
integration of data call files using DATSU. The habitat database is currently
