...
::: notes
The Habitat Database is composed of a river network, with river segments and catchments, and some referentials to create a hierarchical structure. I am going to go over the hierarchical structure today and tomorrow we will show you examples of how it can be used. Everything is currently available on Zenodo if you want to have a look or try it out. I also made a webinar on how to access and use the data. Everything is available on the DIASPARA website
:::
...
::: notes
In order to work more efficently within the habitat database we decided to create a hierarchical structure. It goes from the highest level, which is the whole stock, to either river sections or divisions at sea. Each working group has its own structure but they share some similarities. They all have Stock levels and a country level. I'm going to go over the structure for each working group, starting with WGBAST where I'll go more into details so you can understand how it works.
:::
...
::: notes
The table looks like this. On top you have the whole stock, then the marine stock and the last is the inland one. Each row has an id, an are_code which is either a name or an code for your level, then you have a are_lev_code which represent the hierarchical level, after there is the are_wkg_code which is the working group, then the are_ismarine to know if your working with inland or at sea data, then the are_name which will be the name of the data. Finally, going back to the second column, which is the most important to our hierarchy, is the are_are_id. For each data, the are_are_id refers to its higher hierarchical level. For instance, here the highest level is the whole stock which has an are_id of 1. That's why the are_are_id of both marine stock and inland stock is 1.
:::
...
::: notes
Here we only see the main stretch of river since what I'm showing right now is only the hierarchical structure. But within the DB, all tributaries are included. They are linked to the main stretch by an id. Also, within the RDBES the basin code, which is unique and the name of the river will be included.
:::
...
::: notes
The first level we are going to focus on is the Regional one. It is present in both Marine and Inland parts. We decided to create this level because EMUs are to be deprecated so we needed another structure to assess data. We discussed about this and worked on it during the WGEEL meeting last October in Rennes.
:::
...
::: notes
The way this level has been built was first to create a Regional level at sea. Then, the inland level was built agregating all catchments that are draining in said Marine regional level. Here you can see the grouping that has been decided for the Baltic. Even if for now data is reported for the whole Baltic Sea but it might change to a finer grain in the future. This will have to be discussed within the WG.
:::
...
::: notes
So going inside a complex it looks like this. Here we are around Martigues in France, and this complex groups 4 lagoons together. This structure allows you to report data either to the smallest level as shown here or to the complex if data is not precise enough.
:::
