---
title: "From local to large scale: a habitat database alongside a dam and an electrofishing database to shift from continental habitats to population or panpopulation scales"
author: "Cédric Briand, Jules Oliviéro, Jani Helminen"
fontsize: 20pt
filters:
  - quarto-kroki
title-slide-attributes:
  data-background-image: images/diaspara_background.png
  data-background-size: stretch
  data-background-opacity: "0.5"
logo: images/diaspara_participants_logo_withEUlogo.png
footer: ""
format: 
  revealjs :
    self-contained: true
theme: solarized
background-transition: fade
editor: visual
---

```{r init}
#| echo: FALSE
#| warning: FALSE
#| message: FALSE
#| results: 'hide'

#if (!grepl("montepomi", getwd())) {
if(Sys.info()[["user"]] == 'joliviero'){
setwd("D:/workspace/DIASPARA_WP3_migdb/R")
datawd <- "D:/DIASPARA/wgbast"
} else if (Sys.info()[["user"]] == 'cedric.briand'){
setwd("C:/workspace/DIASPARA_WP3_migdb/R")
datawd <- "C:/Users/cedric.briand/OneDrive - EPTB Vilaine/Projets/DIASPARA/wgbast"
}
source("utilities/load_library.R")
load_library("tidyverse")
load_library("knitr")
load_library("kableExtra")
load_library("icesVocab")
load_library("readxl")
load_library("janitor")
load_library("skimr")
load_library("RPostgres")
load_library("yaml")
load_library("DBI")
load_library("ggplot2")
load_library("sf")
load_library("janitor") #
load_library("htmltools") 
load_library("leaflet")
cred <- read_yaml("../credentials.yml")
# con_diaspara <- dbConnect(Postgres(), 
#                            dbname = cred$dbnamehydro,
#                            host = cred$hostmercure,
#                            port = cred$port,
#                            user = cred$usermercure,
#                            password = cred$passwordmercure)
con_diaspara <- dbConnect(Postgres(), 
                          dbname = cred$dbnamediaspara,
                          host = cred$hostdistant,
                          port = cred$port,
                          user = cred$userdiaspara,
                          password = cred$passworddiaspara)

```

## This presentation

::::: columns
::: {.column .incremental width="40%"}
![](images/clipboard-550275409.png "Eel and brown trout"){width="370"} ![](images/clipboard-639830824.png "electrofishing"){width="370"}
:::

::: {.column width="60%"}
[**Database development and:**]{style="font-size:45px"}

-   [**Regional and Global (Spatial) Models**]{style="font-size:40px"}

-   [**Habitat data**]{style="font-size:40px"}

-   [**Migration obstacles**]{style="font-size:40px"}

-   [**Electrofishing**]{style="font-size:40px"}

-   [**DIADROMOUS DATABASE (Metric DB, Stock DB)**]{style="font-size:40px"}
:::
:::::

## This presentation

::::: columns
::: {.column .incremental width="40%"}
![](images/clipboard-550275409.png "Eel and brown trout"){width="370"} ![](images/clipboard-639830824.png "electrofishing"){width="370"}
:::

::: {.column width="60%"}
-   [**Regional and Global (Spatial) Models**]{style="font-size:45px"}

A big reason why we needed habitat database
:::
:::::

# Taking different regions into account

**Remember from previous presentations:**

![](images/clipboard-1502028863.png)

# What amazing species! What a scientific challenge!

**Remember from previous presentations:**

-   Nested dynamics: pooled marine phase, independent continental phase
-   Local pressures can have consequences at larger scale
    -   Management is often local: consequences at larger scale
    -   Assessment of management actions
-   Large scale pressure can impact local dynamics (e.g. marine fisheries)

# Scale of each model? Not known

::::: columns
::: {.column .incremental width="50%"}
-   In the past, only marine habitat available -\> now marine and continental
-   With this approach we can test different structures and levels
-   Data collected locally, used in (m)any needed scale(s)
-   Data structured in regional level for SAM: all have this structure available
-   Need upscaling: need upscaling methods
-   Identify similar areas, identify different areas
:::

::: {.column width="50%"}
![](images/clipboard-3523976769.png)
:::
:::::

# The spatial / habitat model?

::::: columns
::: {.column .incremental width="50%"}
-   There is a need to have habitat models everywhere
-   Regional aspects in WGBAST, WGNAS and WGBAST
-   [WGTRUTTA](https://diaspara.bordeaux-aquitaine.inrae.fr/deliverables/wp3/p6/wgtrutta_database_description.html)
:::

::: {.column width="50%"}
![](images/clipboard-1441936957.png)
:::
:::::

# Regional aspects, EDA examples

::::: columns
::: {.column .incremental width="40%"}
![](images/clipboard-1546341554.png)
:::

::: {.column width="60%"}
![](images/clipboard-3112802258.png)
:::
:::::

# Regional aspects, EDA examples


-   Hydrographic network, obstacles and electrofishing information needed!

::: {.r-stack}

![](images/clipboard-1171006118.png){.fragment height="300"} 

![](images/clipboard-2093853961.png){.fragment height="400"} 

:::


# Regional aspects, EDA examples

-   To quantify mortality linked with dams (downstream migration):

    ![](images/clipboard-373049954.png)

# Series, Biometrics, and Spatial Aspects

# Example, WGEEL glasseel series

![](images/clipboard-142218191.png)

# Example, WGEEL glasseel series

![](images/clipboard-1391266458.png)

# Example, WGEEL glasseel series

![](images/clipboard-4106761349.png)

# Example, WGEEL glasseel series

![](images/clipboard-3678211794.png)

# Similarly in WGBAST

# ![](images/clipboard-322677207.png)

# Similarly in WGBAST![](images/clipboard-2918597239.png)

# Any other series similarly!

::::: columns
::: {.column .incremental width="40%"}

![](images/clipboard-1432957746.png)
:::

::: {.column width="60%"}
![](images/clipboard-1142296634.png)
:::
:::::

# Biometrics and spatial aspects

![](images/clipboard-961064370.png)

# Biometrics and spatial aspects

![](images/clipboard-3844943477.png)

# Biometrics and spatial aspects

![](images/clipboard-1163778959.png)

# Biometrics and spatial aspects

![](images/clipboard-2272436302.png)

## This presentation

::::: columns
::: {.column .incremental width="40%"}
![](images/clipboard-550275409.png "Eel and brown trout"){width="370"} ![](images/clipboard-639830824.png "electrofishing"){width="370"}
:::

::: {.column width="60%"}
[**Database development and:**]{style="font-size:45px"}

-   [**Habitat data**]{style="font-size:40px"}

The Base for everything
:::
:::::

# Habitat DB

::::: columns
::: {.column width="40%"}
![Habitat Database](images/fig_final_allcatchments.png){#fig-hdb}
:::
::: {.column .incremental width="60%"}
- River network and catchments
- Hierchical structure
- Everything is on Zenodo (https://zenodo.org/records/15856191)
:::
::::

::: notes
The Habitat Database is composed of a river network, with river segments and catchments, and some referentials to create a hierarchical structure. I am going to go over the hierarchical structure today and tomorrow we will show you examples of how it can be used.
Everything is currently available on Zenodo if you want to have a look or try it out. I also made a webinar on how to access and use the data. Everything is available on the DIASPARA website
:::

## Habitat DB hierarchical structure

![WGBAST Hierarchical structure](images/fig-area_hierarchy_bast.svg){#fig-hierbast}

::: {.notes}
In order to work more efficently within the habitat database we decided to create a hierarchical structure. It goes from the highest level, which is the whole stock, to either river sections or divisions at sea.
Each working group has its own structure but they share some similarities. They all have Stock levels and a country level.
I'm going to go over the structure for each working group, starting with WGBAST where I'll go more into details so you can understand how it works.
:::

## Habitat DB hierarchical structure : Stock

![WGBAST Stock level](images/fig-area_hierarchy_bast_stock.svg){#fig-hierbastsck}

::: notes
The first level is the Stock level. You have three layers here. The whole stock that is composed on one side of the marine stock and the inland stock on the other.
:::

## Habitat DB hierarchical structure : Stock

```{r}
#| label: refbast.tr_area_are Stock level
#| echo: FALSE
#| warning: FALSE
#| message: FALSE
#| tbl-cap: Structure of the global Stock level in the referential table


fstock_lvl <- dbGetQuery(con_diaspara, 
  "SELECT are_id, are_are_id, are_code, are_lev_code, are_wkg_code, are_ismarine FROM refbast.tr_area_are
  WHERE are_lev_code = 'Stock'
  ORDER BY are_id;")

DT::datatable(fstock_lvl,
rownames = FALSE,
  options = list(
    pageLength = 7,        
    scrollX= TRUE   
  )
) |>
  htmltools::tagList(

 
    htmltools::tags$style(HTML("
      /* Table cells */
      table.dataTable td {
        font-size: 0.6em;
      }
 
      /* Table headers */
      table.dataTable th {
        font-size: 0.65em;
      }
 
      /* Control bar: search input, pagination, etc. */
      .dataTables_wrapper .dataTables_filter,
      .dataTables_wrapper .dataTables_length,
      .dataTables_wrapper .dataTables_info,
      .dataTables_wrapper .dataTables_paginate {
        font-size: 0.70em;
      }
    ")))

```

::: notes
The table looks like this. On top you have the whole stock, then the marine stock and the last is the inland one.
Each row has an id, an are_code which is either a name or an code for your level, then you have a are_lev_code which represent the hierarchical level, after there is the are_wkg_code which is the working group, then the are_ismarine to know if your working with inland or at sea data, then the are_name which will be the name of the data. Finally, going back to the second column, which is the most important to our hierarchy, is the are_are_id. For each data, the are_are_id refers to its higher hierarchical level. For instance, here the highest level is the whole stock which has an are_id of 1. That's why the are_are_id of both marine stock and inland stock is 1.
:::

## Habitat DB hierarchical structure : Stock

![](images/bast_stock.png "WGBAST Stock level")

::: notes
For each level you have a geometry attached to it. Here is what the stock levels looks like. It represents the whole area of interest for WGBAST, being inland or at sea.
:::

## Habitat DB hierarchical structure : Assessment_unit

![WGBAST Assesment_unit level](images/fig-area_hierarchy_bast_au.svg){#fig-hierbastau}

::: notes
First we will go deeper into the inland levels. The first one is the Assessment Units used by WGBAST.
:::

## Habitat DB hierarchical structure : Assessment_unit

```{r}
#| label: refbast.tr_area_are assessment unit level
#| echo: FALSE
#| warning: FALSE
#| message: FALSE
#| tbl-cap: Structure of the assessment unit level in the referential table


fstock_lvl <- dbGetQuery(con_diaspara, 
  "SELECT are_id, are_are_id, are_code, are_lev_code, are_wkg_code, are_ismarine FROM refbast.tr_area_are
  WHERE are_id = ANY(ARRAY[1,3]) OR are_lev_code = 'Assessment_unit' 
  ORDER BY are_id;")

DT::datatable(fstock_lvl,
rownames = FALSE,
  options = list(
    pageLength = 7,        
    scrollX= TRUE   
  )
) |>
  htmltools::tagList(

 
    htmltools::tags$style(HTML("
      /* Table cells */
      table.dataTable td {
        font-size: 0.6em;
      }
 
      /* Table headers */
      table.dataTable th {
        font-size: 0.65em;
      }
 
      /* Control bar: search input, pagination, etc. */
      .dataTables_wrapper .dataTables_filter,
      .dataTables_wrapper .dataTables_length,
      .dataTables_wrapper .dataTables_info,
      .dataTables_wrapper .dataTables_paginate {
        font-size: 0.70em;
      }
    ")))

#knitr::kable(fstock_lvl) %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"))

```

::: notes
There is 6 of them, each is refering to the inland stock with a are_are_id of 3.
:::

## Habitat DB hierarchical structure : Assessment_unit

::::: columns
::: {.column .incremental width="50%"}
-   Gap below Assessment Unit 3
-   No Assessment Unit on the Western part
:::

::: {.column width="50%"}
![](images/bast_au.png "WGBAST Assessment_unit level")
:::
:::::

::: notes
The Assessment units look like this. There is two things that we noticed though. All catchments in the Baltic are not represented. There is a gap bellow the Assessment_unit 3 and there is no AU on the Western part of the Baltic. So we were actually wondering if that was something that might need to be added in the future ?
:::

## Habitat DB hierarchical structure : River

![WGBAST River level](images/fig-area_hierarchy_bast_riv.svg){#fig-hierbastriv}

::: notes
Going lower is the River level, which are composed of river basins.
:::

## Habitat DB hierarchical structure : River

```{r}
#| label: refbast.tr_area_are main river level
#| echo: FALSE
#| warning: FALSE
#| message: FALSE
#| tbl-cap: Structure of the main river level in the referential table


fstock_lvl <- dbGetQuery(con_diaspara, 
  "SELECT are_id, are_are_id, are_code, are_lev_code, are_wkg_code, are_ismarine, are_name FROM refbast.tr_area_are
  WHERE are_id = ANY(ARRAY[1,3,13]) OR are_are_id = 13
  ORDER BY are_id;")

DT::datatable(fstock_lvl,
rownames = FALSE,
  options = list(
    pageLength = 7,        
    scrollX= TRUE   
  )
) |>
  htmltools::tagList(

 
    htmltools::tags$style(HTML("
      /* Table cells */
      table.dataTable td {
        font-size: 0.6em;
      }
 
      /* Table headers */
      table.dataTable th {
        font-size: 0.65em;
      }
 
      /* Control bar: search input, pagination, etc. */
      .dataTables_wrapper .dataTables_filter,
      .dataTables_wrapper .dataTables_length,
      .dataTables_wrapper .dataTables_info,
      .dataTables_wrapper .dataTables_paginate {
        font-size: 0.70em;
      }
    ")))

#knitr::kable(fstock_lvl) %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"))

```

::: notes
This time each River basin references to its assessment unit. Here is the example for the fist AU which as an are_id of 13.
:::

## Habitat DB hierarchical structure : River

![](images/bast_river.png "WGBAST River level")

::: notes
It looks like this. And as you can see, names are attached to each basins (when data was available). This could be updated if needed.
:::

## Habitat DB hierarchical structure : River_section

![WGBAST River_section level](images/fig-area_hierarchy_bast_rivsec.svg){#fig-hierbastrvsc}

::: notes
Finally for the inland part is the river_section level.
:::

## Habitat DB hierarchical structure : River_section

```{r}
#| label: refbast.tr_area_are main river_section level
#| echo: FALSE
#| warning: FALSE
#| message: FALSE
#| tbl-cap: Structure of the main river level in the referential table


fstock_lvl <- dbGetQuery(con_diaspara, 
  "SELECT are_id, are_are_id, are_code, are_lev_code, are_wkg_code, are_ismarine, are_name FROM refbast.tr_area_are
  WHERE are_id = ANY(ARRAY[1,3,13,19]) OR are_are_id = 19
  ORDER BY are_id;")

DT::datatable(fstock_lvl,
rownames = FALSE,
  options = list(
    pageLength = 7,        
    scrollX= TRUE   
  )
) |>
  htmltools::tagList(

 
    htmltools::tags$style(HTML("
      /* Table cells */
      table.dataTable td {
        font-size: 0.6em;
      }
 
      /* Table headers */
      table.dataTable th {
        font-size: 0.65em;
      }
 
      /* Control bar: search input, pagination, etc. */
      .dataTables_wrapper .dataTables_filter,
      .dataTables_wrapper .dataTables_length,
      .dataTables_wrapper .dataTables_info,
      .dataTables_wrapper .dataTables_paginate {
        font-size: 0.70em;
      }
    ")))

#knitr::kable(fstock_lvl) %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"))

```

::: notes
Here again, each river segment refers to its river basin.
:::

## Habitat DB hierarchical structure : River_section

![](images/bast_rs.png "WGBAST River_section level")

::: notes
Here we only see the main stretch of river since what I'm showing right now is only the hierarchical structure. But within the DB, all tributaries are included. They are linked to the main stretch by an id.
Also, within the RDBES the basin code, which is unique and the name of the river will be included.
:::

## Habitat DB hierarchical structure : Divison

![WGBAST Division level](images/fig-area_hierarchy_bast_div.svg){#fig-hierbastdiv}

::: notes
Now we are going to jump onto the marine part. The second level is the Division level. It is composed of ICES Division in the Baltic Sea.
:::

## Habitat DB hierarchical structure : Divison

```{r}
#| label: refbast.tr_area_are ICES division level
#| echo: FALSE
#| warning: FALSE
#| message: FALSE
#| tbl-cap: Structure of the ICES division level in the referential table


fstock_lvl <- dbGetQuery(con_diaspara, 
  "SELECT are_id, are_are_id, are_code, are_lev_code, are_wkg_code, are_ismarine FROM refbast.tr_area_are
  WHERE are_id = ANY(ARRAY[1,2]) OR are_lev_code = 'Division'
  ORDER BY are_id;")

DT::datatable(fstock_lvl,
rownames = FALSE,
  options = list(
    pageLength = 7,        
    scrollX= TRUE   
  )
) |>
  htmltools::tagList(

 
    htmltools::tags$style(HTML("
      /* Table cells */
      table.dataTable td {
        font-size: 0.6em;
      }
 
      /* Table headers */
      table.dataTable th {
        font-size: 0.65em;
      }
 
      /* Control bar: search input, pagination, etc. */
      .dataTables_wrapper .dataTables_filter,
      .dataTables_wrapper .dataTables_length,
      .dataTables_wrapper .dataTables_info,
      .dataTables_wrapper .dataTables_paginate {
        font-size: 0.70em;
      }
    ")))

```

::: notes
There is only two of them and they both refer to the Marine Stock with the are_are_id 2.
:::

## Habitat DB hierarchical structure : Divison

![](images/bast_div.png "WGBAST Division level")

## Habitat DB hierarchical structure : Subdivision_grouping

![WGBAST Subdivision_grouping level](images/fig-area_hierarchy_bast_subgrp.svg){#fig-hierbastsubgr}

::: notes
Then we decided to create an intermediate level which is grouping several ICES subdivision together.
:::

## Habitat DB hierarchical structure : Subdivision_grouping

```{r}
#| label: refbast.tr_area_are ICES subdivision_grouping level
#| echo: FALSE
#| warning: FALSE
#| message: FALSE
#| tbl-cap: Structure of the ICES subdivision grouping level in the referential table


fstock_lvl <- dbGetQuery(con_diaspara, 
  "SELECT are_id, are_are_id, are_code, are_lev_code, are_wkg_code, are_ismarine FROM refbast.tr_area_are
  WHERE are_id = ANY(ARRAY[1,2,7011]) OR are_lev_code = 'Subdivision_grouping'
  ORDER BY are_id;")

DT::datatable(fstock_lvl,
rownames = FALSE,
  options = list(
    pageLength = 7,        
    scrollX= TRUE   
  )
) |>
  htmltools::tagList(

 
    htmltools::tags$style(HTML("
      /* Table cells */
      table.dataTable td {
        font-size: 0.6em;
      }
 
      /* Table headers */
      table.dataTable th {
        font-size: 0.65em;
      }
 
      /* Control bar: search input, pagination, etc. */
      .dataTables_wrapper .dataTables_filter,
      .dataTables_wrapper .dataTables_length,
      .dataTables_wrapper .dataTables_info,
      .dataTables_wrapper .dataTables_paginate {
        font-size: 0.70em;
      }
    ")))

#knitr::kable(fstock_lvl) %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"))

```

## Habitat DB hierarchical structure : Subdivision_grouping

::::: columns
::: {.column .incremental width="50%"}
-   Useful for historical data. Used to be grouped in larger areas
:::

::: {.column width="50%"}
![](images/bast_subgr.png "WGBAST Subdivision_grouping level")
:::
:::::

::: notes
It has been done because those grouping were used in the past by WGBAST and are still present in the historical data.
:::

## Habitat DB hierarchical structure : Subdivision

![WGBAST Subdivision level](images/fig-area_hierarchy_bast_subdiv.svg){#fig-hierbastsub}

::: notes
Finally, the last level is the ICES subdivision level.
:::

## Habitat DB hierarchical structure : Subdivision

```{r}
#| label: refbast.tr_area_are ICES subdivision level
#| echo: FALSE
#| warning: FALSE
#| message: FALSE
#| tbl-cap: Structure of the ICES subdivision level in the referential table


fstock_lvl <- dbGetQuery(con_diaspara, 
  "SELECT are_id, are_are_id, are_code, are_lev_code, are_wkg_code, are_ismarine FROM refbast.tr_area_are
  WHERE are_id = ANY(ARRAY[1,2]) OR are_lev_code = ANY(ARRAY['Division', 'Subdivision'])
  ORDER BY are_id;")

# DT::datatable(fstock_lvl,
#                 rownames = FALSE, 
#                 class = "stripe hover compact", 
#                 options = list(dom = 't',
#                           pageLength = 100))

DT::datatable(fstock_lvl,
rownames = FALSE,
  options = list(
    pageLength = 7,        
    scrollX= TRUE   
  )
) |>
  htmltools::tagList(

 
    htmltools::tags$style(HTML("
      /* Table cells */
      table.dataTable td {
        font-size: 0.6em;
      }
 
      /* Table headers */
      table.dataTable th {
        font-size: 0.65em;
      }
 
      /* Control bar: search input, pagination, etc. */
      .dataTables_wrapper .dataTables_filter,
      .dataTables_wrapper .dataTables_length,
      .dataTables_wrapper .dataTables_info,
      .dataTables_wrapper .dataTables_paginate {
        font-size: 0.70em;
      }
    ")))

#knitr::kable(fstock_lvl) %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"))

```

::: notes
Each subdivision refers to its division.
:::

## Habitat DB hierarchical structure : Subdivision

![](images/bast_sub.png "WGBAST Subdivision level")

## Habitat DB hierarchical structure : WGEEL

![WGEEL Hierarchical structure](images/fig-area_hierarchy_wgeel.svg){#fig-hiereel}

::: notes
That was it for WGBAST. Now I'm going to go over the differences in for WGEEL. As stated earlier, the structure is similar but some levels are unique to WGEEL.
:::

## Habitat DB hierarchical structure : Regional

-   EMUs are to be deprecated
-   Creating a new structure to assess data
-   Has been presented and disscussed during WGEEL meeting

::: notes
The first level we are going to focus on is the Regional one. It is present in both Marine and Inland parts.
We decided to create this level because EMUs are to be deprecated so we needed another structure to assess data. We discussed about this and worked on it during the WGEEL meeting last October in Rennes.
:::

## Habitat DB hierarchical structure : Regional

![WGEEL Regional level](images/fig-area_hierarchy_wgeel_reg.svg){#fig-hierregwgeel}

## Habitat DB hierarchical structure : Regional

![](images/wgeel_baltic.png "WGEEL Regional level")

::: notes
The way this level has been built was first to create a Regional level at sea. Then, the inland level was built agregating all catchments that are draining in said Marine regional level.
Here you can see the grouping that has been decided for the Baltic.
Even if for now data is reported for the whole Baltic Sea but it might change to a finer grain in the future. This will have to be discussed within the WG.
:::

## Habitat DB hierarchical structure : Regional

![](images/wgeel_norway.png "WGEEL Regional level")

::: notes
Here is the second region with the Barents and Iceland
:::

## Habitat DB hierarchical structure : Regional

![](images/wgeel_nsea.png "WGEEL Regional level")

::: notes
Then you have the North Sea in Green, part of Great Britain and Ireland in blue and then the Channel in teal.
:::

## Habitat DB hierarchical structure : Regional

![](images/wgeel_atlantic.png "WGEEL Regional level")

::: notes
Next regions are the Bay of Biscay and the Southern part of the Atlantic.
:::

## Habitat DB hierarchical structure : Regional

![](images/wgeel_med.png "WGEEL Regional level")

::: notes
And finally we have the Mediterranean which is split in three, with the Adriatic and the Black Sea as their own regions.
:::

## Habitat DB hierarchical structure : Complex
![WGEEL Complex level](images/fig-area_hierarchy_wgeel_comp.svg){#fig-hiercompwgeel}

## Habitat DB hierarchical structure : Complex

-   Taking care of lagoons in the Mediterranean
-   Creating groups of lagoons
-   Available for Spain, France, Tunisia, Italia, Albania, Greece and Turkey

::: notes
In the Meditarranean lagoons play an important role for eels. Lagoons from Spain, France, Tunisia, Italia, Albania, Greece and Turkey are included in the database. And because of the way data is reported in some countries we had to create groups of lagoons that we named Complex.
:::

## Habitat DB hierarchical structure : Complex

![](images/greece_complex.png "WGEEL Complex level")

## Habitat DB hierarchical structure : Complex

![](images/french_complex.png "WGEEL Complex level")

## Habitat DB hierarchical structure : Lagoons

![WGEEL Lagoon level](images/fig-area_hierarchy_wgeel_lag.svg){#fig-hierlagwgeem}

## Habitat DB hierarchical structure : Lagoons

![](images/wgeel_lag.png "WGEEL Lagoons level")

::: notes
So going inside a complex it looks like this. Here we are around Martigues in France, and this complex groups 4 lagoons together.
This structure allows you to report data either to the smallest level as shown here or to the complex if data is not precise enough.
:::

## Habitat DB hierarchical structure : Lagoons

-   Lakes are to be added as well

## Habitat DB hierarchical structure : WGNAS

![WGNAS Hierarchical structure](images/fig-area_hierarchy_nas1.svg){#fig-hiernas}

::: notes
Finally we are moving to WGNAS. The main difference here is the importance of Assessment Units, both inland and at sea and the presence of Fisheries.
:::

## Habitat DB hierarchical structure : Assessment_unit

![WGNAS AU level](images/fig-area_hierarchy_nasau.svg){#fig-hiernasau}

## Habitat DB hierarchical structure : Assessment_unit

::::: columns
::: {.column .incremental width="40%"}
- North American Assessment units are included
-   Electrofishing data will have to be reported at this level
:::

::: {.column width="60%"}
![](images/nas_aunit.png "WGNAS AU level")
:::
:::::

::: notes
Also as you can see, North A. has been included in the database. We tried to recreate marine AU as close as possible to the ones used by WGNAS using ICES divisions and subdivisions.
For now, electrofishing data will have to be reported at this level. Although, after talking with Etienne, we understood that in the future smaller AU might be used by WGNAS
:::

## Habitat DB hierarchical structure : River

![WGNAS River level](images/fig-area_hierarchy_nasriv.svg){#fig-hiernasriv}

## Habitat DB hierarchical structure : River
::::: columns
::: {.column .incremental width="50%"}
-   Possibility to work with river basins to work with smaller AU
:::

::: {.column width="50%"}
![](images/nas_riv.png "WGNAS River level")
:::
:::::

::: notes
To do so, the River level is available. By gathering several basin together, creating smaller assessment units is possible.
:::

## Habitat DB hierarchical structure : River

::::: columns
::: {.column .incremental width="40%"}
-   Possibility to group river basins to create smaller assessment units
-   Different pressures, quantification issues?
    -   Obstacles, turbines matched to habitat
:::

::: {.column width="60%"}
![](images/nas_rivseg.png "WGNAS River_segment level")
:::
:::::

## This presentation

::::: columns
::: {.column .incremental width="40%"}
![](images/clipboard-550275409.png "Eel and brown trout"){width="370"} ![](images/clipboard-639830824.png "electrofishing"){width="370"}
:::

::: {.column width="60%"}
[**Database development and:**]{style="font-size:45px"}

-   [**Migration obstacles**]{style="font-size:40px"}

For habitat availability, mortality, ...
:::
:::::

# DAM DATABASE

::::: columns
::: {.column width="50%"}
```{mermaid}
%%| fig-cap: Simplied structure of the dam database
flowchart TB
    A([Obstruction place]):::table
    %% Short Path
    A --> B([Obstruction]):::table
    B -->|Upstream| F([fish passage]):::table
    B -->|Downstream| C([Hydro Power Plant]):::table
    C --> D([Turbine]):::table    
    classDef table fill:#CD88
    classDef main fill:#268073,color:white
```
:::

::: {.column width="50%"}
![](images/dam_spain.jpg)
:::
:::::

## DAM DATABASE : Obstruction place

::::: columns
::: {.column width="50%"}
```{mermaid}
%%| fig-cap: Simplied structure of the dam database
flowchart TB
    A([Obstruction place]):::main
    %% Short Path
    A --> B([Obstruction]):::table
    B -->|Upstream| F([fish passage]):::table
    B -->|Downstream| C([Hydro Power Plant]):::table
    C --> D([Turbine]):::table    
    classDef table fill:#CD88
    classDef main fill:#268073,color:white
```
:::

::: {.column width="50%"}
-   Obstruction place is a point on the map
-   It has a hierarchy (dam related to other dams in braided network)
-   It belongs to a country (by inheritance), so dam data are filled in and replaced at the national level
-   Class following AMBER + sudoang

```{mermaid}

flowchart TB
    A([Obstruction place]):::table
    %% Short Path
    A --> B([Obstuction place FR]):::table
    A --> C([Obstuction place ES]):::table 
    A --> D([Obstuction place ...]):::table      
    classDef table fill:#CD88
    classDef main fill:#268073,color:white
```
:::
:::::

## DAM DATABASE : Obstruction place

```{r}
#| label: tbl-obstruction_place
#| echo: FALSE
#| eval: TRUE
#| warning: FALSE
#| html-table-processing: none


# dam <-dbGetQuery(con_diaspara, "SELECT 
# op_placename,
# op_op_id,
# op_id_original,
# op_country
#  FROM dam_france.obstruction_place limit 500;")

# save(dam, file="data/presentation_dam_op.Rdata")
load(file="data/presentation_dam_op.Rdata")

 dam |> 
 DT::datatable(rownames = FALSE,
  options = list(
    pageLength = 7,        
    scrollX= TRUE   
  )
) |>
  htmltools::tagList(
   

    tags$style(HTML("
      /* Table cells */
      table.dataTable td {
        font-size: 0.6em;
      }

      /* Table headers */
      table.dataTable th {
        font-size: 0.65em;
      }

      /* Control bar: search input, pagination, etc. */
      .dataTables_wrapper .dataTables_filter,
      .dataTables_wrapper .dataTables_length,
      .dataTables_wrapper .dataTables_info,
      .dataTables_wrapper .dataTables_paginate {
        font-size: 0.70em;
      }
    "))

  )

```

## DAM DATABASE : Obstruction place

```{r}
#| label: fig-obstruction_place
#| echo: FALSE
#| eval: TRUE
#| warning: FALSE
#| fig-cap: "A subsample of the dam database in France"

# query <- "SELECT op_id_original,
#                  op_placename, 
#                  ob_height, 
#                  ot.no_code as type,                 
#                  geom 
#           FROM dam_france.obstruction_place op
#           JOIN dam_france.obstruction o
#              ON ob_op_id = op_id
#           JOIN nomenclature.obstruction_type ot
#              ON ob_obstruction_type_no_id = ot.no_id
#           JOIN refeel.tr_area_are are ON 
#           st_intersects(st_transform(op.geom, 4326), are.geom_polygon)
#           WHERE are_code = 'FR_Bret'
#           AND ob_ending_date = '2035-01-01'
#           "
# query2 <- "SELECT are1.are_id, are1.geom_line FROM refnas.tr_area_are are1 
#           JOIN refeel.tr_area_are are2 ON 
#           st_intersects(are1.geom_line, are2.geom_polygon)
#           WHERE are2.are_code = 'FR_Bret'
#           AND  are1.are_lev_code = 'River_section'"


# layer <- st_read(dsn = con_diaspara, query = query) |> 
#          sf::st_transform(4326) 
# layer_riv <- st_read(dsn = con_diaspara, query = query2) 



# save(layer, layer_riv, file = "data/dammap_presentation.Rdata")
load(file = "data/dammap_presentation.Rdata")
layer <- layer |>
          mutate(label = sprintf(
            '%s <br>	
             code: %s <br>
             type: %s <br>				
					   h: %1.2f,<br> ',
            stringi::stri_trans_general(layer$op_placename, "latin-ascii"),
            layer$op_id_original,
            layer$type,            
            layer$ob_height
          )) 
layer$size <-  1+log(layer$ob_height+1) #  range 1-13
layer$size[is.na(layer$size)] <- 0.01
layer$type[is.na(layer$type)]<-"NA"
layer$type <- as.factor(layer$type)
cols <- RColorBrewer::brewer.pal(length(levels(layer$type)),name ="Set1")
colorfun <- leaflet::colorFactor(cols, layer$type)


l <- leaflet::leaflet() |>
     leaflet::addPolylines(data= layer_riv, 
     opacity=0.5,
     fillOpacity = 0.2
     ) |>
     leaflet::addScaleBar(options = leaflet::scaleBarOptions(imperial = FALSE)) |>
     leaflet::addProviderTiles(leaflet::providers$CartoDB.DarkMatter) |>
     leaflet::addCircleMarkers(
        data = layer[1:500,],
        popup = ~label,
        color = ~colorfun(type),       
        radius = ~size       
      ) |>
      leaflet::addLegend(
        position="topright",
        colors = cols,
        labels = levels(layer$type),
        opacity = 1)
    
l

```

## DAM DATABASE : Physical obstruction

::::: columns
::: {.column width="50%"}
```{mermaid}
%%| fig-cap: Simplied structure of the dam database
flowchart TB
    A([Obstruction place]):::table
    %% Short Path
    A --> B([Obstruction]):::main
    B -->|Upstream| F([fish passage]):::table
    B -->|Downstream| C([Hydro Power Plant]):::table
    C --> D([Turbine]):::table    
    classDef table fill:#CD88
    classDef main fill:#268073,color:white
```
:::

::: {.column width="50%"}
-   Physical obstruction has date of start and end
-   Events (Erasement, change of height change dam period)
-   Fishway types
-   Downstream water depth
:::
:::::

## DAM DATABASE : Physical obstruction

```{r}
#| label: tbl-obstruction
#| echo: FALSE
#| eval: TRUE
#| warning: FALSE
#| html-table-processing: none


# dam_ob <-dbGetQuery(con_diaspara, "SELECT 
# * FROM dam_france.obstruction limit 100;")

# save(dam_ob, file="data/presentation_dam_ob.Rdata")
load(file="data/presentation_dam_ob.Rdata")

 dam_ob |> 
 DT::datatable(rownames = FALSE,
  options = list(
    pageLength = 5,        
    scrollX= TRUE   
  )
)|>
  htmltools::tagList(
   

     htmltools::tags$style(HTML("
      /* Table cells */
      table.dataTable td {
        font-size: 0.6em;
      }

      /* Table headers */
      table.dataTable th {
        font-size: 0.65em;
      }

      /* Control bar: search input, pagination, etc. */
      .dataTables_wrapper .dataTables_filter,
      .dataTables_wrapper .dataTables_length,
      .dataTables_wrapper .dataTables_info,
      .dataTables_wrapper .dataTables_paginate {
        font-size: 0.70em;
      }
    "))

  )

```

## DAM DATABASE : assessment of passability

```{mermaid}
%%| fig-cap: Simplied structure of the dam database
flowchart TB
    A([Obstruction place]):::table
    %% Short Path
    A --> B([Obstruction]):::table
    B -->|Upstream| F([fish passage]):::main
    B -->|Downstream| C([Hydro Power Plant]):::table
    C --> D([Turbine]):::table    
    classDef table fill:#CD88
    classDef main fill:#268073,color:white
```

## DAM DATABASE : assessment of passability

```{r}
#| label: tbl-fishway
#| echo: FALSE
#| eval: TRUE
#| warning: FALSE
#| html-table-processing: none


 dam_fi <-dbGetQuery(con_diaspara, "SELECT 
 fi_id_original, 
 species.no_name,
 fi_presence_pass,
 fi_date
  FROM dam_france.fishway  
  JOIN nomenclature.species on species.no_id=fi_species_id
  limit 100;")

 save(dam_fi, file="data/presentation_dam_fi.Rdata")
load(file="data/presentation_dam_fi.Rdata")

 dam_fi |> 
 DT::datatable(rownames = FALSE,
  options = list(
    pageLength = 5,        
    scrollX= TRUE   
  )
)|>
  htmltools::tagList(
   

     htmltools::tags$style(HTML("
      /* Table cells */
      table.dataTable td {
        font-size: 0.6em;
      }

      /* Table headers */
      table.dataTable th {
        font-size: 0.65em;
      }

      /* Control bar: search input, pagination, etc. */
      .dataTables_wrapper .dataTables_filter,
      .dataTables_wrapper .dataTables_length,
      .dataTables_wrapper .dataTables_info,
      .dataTables_wrapper .dataTables_paginate {
        font-size: 0.70em;
      }
    "))

  )

```

## DAM DATABASE : Hydro Power Plants

```{mermaid}
%%| fig-cap: Simplied structure of the dam database
flowchart TB
    A([Obstruction place]):::table
    %% Short Path
    A --> B([Obstruction]):::table
    B -->|Upstream| F([fish passage]):::table
    B -->|Downstream| C([Hydro Power Plant]):::main
    C --> D([Turbine]):::table    
    classDef table fill:#CD88
    classDef main fill:#268073,color:white
```

## DAM DATABASE : Hydro Power

```{r}
#| label: tbl-hpp
#| echo: FALSE
#| eval: TRUE
#| warning: FALSE
#| html-table-processing: none
#| tbl-cap: Hydropower plant table


 dam_hpp <-dbGetQuery(con_diaspara, "SELECT 
 *
  FROM dam_france.hpp   
  limit 100;")

 save(dam_hpp, file="data/presentation_dam_hpp.Rdata")
load(file="data/presentation_dam_hpp.Rdata")

 dam_hpp |> 
 DT::datatable(rownames = FALSE,
  options = list(
    pageLength = 5,        
    scrollX= TRUE   
  )
)|>
  htmltools::tagList(
   

     htmltools::tags$style(HTML("
      /* Table cells */
      table.dataTable td {
        font-size: 0.6em;
      }

      /* Table headers */
      table.dataTable th {
        font-size: 0.65em;
      }

      /* Control bar: search input, pagination, etc. */
      .dataTables_wrapper .dataTables_filter,
      .dataTables_wrapper .dataTables_length,
      .dataTables_wrapper .dataTables_info,
      .dataTables_wrapper .dataTables_paginate {
        font-size: 0.70em;
      }
    "))

  )

```

## DAM DATABASE : Turbines

```{mermaid}
%%| fig-cap: Simplied structure of the dam database
flowchart TB
    A([Obstruction place]):::table
    %% Short Path
    A --> B([Obstruction]):::table
    B -->|Upstream| F([fish passage]):::table
    B -->|Downstream| C([Hydro Power Plant]):::table
    C --> D([Turbine]):::main    
    classDef table fill:#CD88
    classDef main fill:#268073,color:white
```

## DAM DATABASE : Turbines

::::: columns
::: {.column width="50%"}
```{r}
#| label: tbl-turbines
#| echo: FALSE
#| eval: TRUE
#| warning: FALSE
#| html-table-processing: none
#| tbl-cap: Hydropower plant table


 dam_turb <-dbGetQuery(con_diaspara, "SELECT 
 *
  FROM dam_france.turbine  
  limit 100;")

 save(dam_turb, file="data/presentation_dam_turb.Rdata")
load(file="data/presentation_dam_turb.Rdata")

 dam_turb |> 
 DT::datatable(rownames = FALSE,
  options = list(
    pageLength = 5,        
    scrollX= TRUE   
  )
)|>
  htmltools::tagList(
   

     htmltools::tags$style(HTML("
      /* Table cells */
      table.dataTable td {
        font-size: 0.6em;
      }

      /* Table headers */
      table.dataTable th {
        font-size: 0.65em;
      }

      /* Control bar: search input, pagination, etc. */
      .dataTables_wrapper .dataTables_filter,
      .dataTables_wrapper .dataTables_length,
      .dataTables_wrapper .dataTables_info,
      .dataTables_wrapper .dataTables_paginate {
        font-size: 0.70em;
      }
    "))

  )

```
:::

::: {.column width="50%"}
<br> <br>

```{r}
#| label: tbl-turbines_type
#| echo: FALSE
#| eval: TRUE
#| warning: FALSE
#| html-table-processing: none
#| tbl-cap: Hydropower plant table


 turb_type <-dbGetQuery(con_diaspara, "SELECT 
 *
  FROM nomenclature.turbine_type;")

 save(turb_type, file="data/presentation_turb_type.Rdata")
load(file="data/presentation_turb_type.Rdata")

 turb_type |> 
 DT::datatable(rownames = FALSE,
  options = list(
    pageLength = 5,        
    scrollX= TRUE   
  )
)|>
  htmltools::tagList(
   

     htmltools::tags$style(HTML("
      /* Table cells */
      table.dataTable td {
        font-size: 0.6em;
      }

      /* Table headers */
      table.dataTable th {
        font-size: 0.65em;
      }

      /* Control bar: search input, pagination, etc. */
      .dataTables_wrapper .dataTables_filter,
      .dataTables_wrapper .dataTables_length,
      .dataTables_wrapper .dataTables_info,
      .dataTables_wrapper .dataTables_paginate {
        font-size: 0.70em;
      }
    "))

  )

```
:::
:::::

# Electrofishing

::::: columns
::: {.column .incremental width="40%"}
![](images/clipboard-2480369358.png){width="484"}
:::

::: {.column width="60%"}

-   Electrofishing development halfway done (SUDOANG)

-   We are working on this db now

-   Templates to be generated and discussed
:::
:::::

## This presentation

::::: columns
::: {.column .incremental width="40%"}
![](images/clipboard-550275409.png "Eel and brown trout"){width="370"} ![](images/clipboard-639830824.png "electrofishing"){width="370"}
:::

::: {.column width="60%"}
[**Database development and:**]{style="font-size:45px"}

-   [**DIADROMOUS DATABASE (Metric DB, Stock DB)**]{style="font-size:40px"}

Where the fish data go

:::
:::::

## DIADROMOUS DATABASE (Stock DB)

-   Structuration of data at the stock / assessment Unit level.

![](images/DIAD1.png "Stock DB")

## DIADROMOUS DATABASE (Metric DB)

![](images/DIAD2.1.png "Metric DB - detail of tables")

## DIADROMOUS DATABASE (Stock DB)

![](images/DIAD1.1.png "Stock DB - Data input")

## DIADROMOUS DATABASE (Stock DB)

![](images/DIAD1.3.png "Stock DB - metadata")

## DIADROMOUS DATABASE (Stock DB)

![](images/DIAD1.4.png "Stock DB - metadata")

## DIADROMOUS DATABASE (Summary)

![](images/DIAD3.png "Stock DB - summary")

## DIADROMOUS DATABASE (Data Flow)

![](images/processes.png "Data flow")

# Thank you!
