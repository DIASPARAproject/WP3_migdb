...
con <- dbConnect(drv, host="localhost", user="atlas", password="atlas", dbname="salmoglob_diaspara")
...
body <-
  dashboardBody(
      ...
      tabItem (
        ...
        fluidPage(
          ...
          box(
            ...
            br(),
            h5("The ", strong("WGNAS-SalmoGlob ToolBox"), "supports and promotes Atlantic salmon stock assessment by the",
          ...
          box(
            ...
            p("This application was developed as part of the SalmoGlob project, led by Etienne Rivot, Pierre-Yves Hernvann, Maxime Olmos, Rémi Patin Jérôme Guitton",
            ...
            column(
              ...
              column(
                ...
                selectInput(
                  ...
                  choices = c("","Returns","Homewater catches","Sea catches",
            ...
            fluidRow(
              ...
              tabBox(
                ...
                tabPanel(
                  ...
                  headerPanel(
                    tags$img(
                      "Download data call templates",src="ices_full_logo_transparent.png", 
                      height="60", 
                      align="right"
                    )
                  ),
    ...
    tags$head(tags$link(rel = "stylesheet", type = "text/css", href = "design.css" # This line call a .css file managing the overall aesthetic of the App. The aesthetics could also be managed within this script but it would 
...
server <- function(input, output, session) {
  ...
  next_trans<-reactive({as.numeric(input$next_st)})
  previous_trans<-reactive({as.numeric(input$previous_st)})
  ...
  calcdiff <- reactive({(next_trans()-previous_trans()+1)-reset_navig()})
  ...
  newdata<-reactive({
    ...
    oups <- cbind.data.frame(data_to_update, as.character(input$Name), as.character(input$Firstname),####
  ...
  observeEvent(input$activ_emerg, {
    ...
    updateRadioButtons(session, "sep", label = "Separator", choices = c(Comma = ",",Semicolon = ";",Tab = "\t"),
                       selected = "", inline=T)
    delay(1000, updateRadioButtons(session, "sep", label = "Separator", choices = c(Comma = ",",Semicolon = ";",Tab = "\t"),
                                   selected = ",", inline=T))
    #updateRadioButtons(session, "sep", label = "Separator", choices = c(Comma = ",",Semicolon = ";",Tab = "\t"),
    #                   selected = ",", inline=T)
  })
  ...
  agetype <- reactive({
    ...
    }else{tt<-"Select salmon sea winter ages"}
    return(tt)
  })
  ...
  observeEvent(eventExpr = input$datatype_explo, handlerExpr = {
    if(!input$datatype_explo %in% c("Time spent at sea","Survival rate","Origin distribution in sea catches",
  ...
  observeEvent(eventExpr = input$area_out, handlerExpr = {
    if(input$datatype_out %in% c("Sea catches") & input$area_out %in% c("GLD fishery", "FAR fishery")) {
      ...
      updateRadioButtons(session, "var_hind", label = "Catch data to plot", choices = c("Total catch" = "tot", "SU proportions" = "prop"),
  ...
  output$box_max <- renderUI({
    ...
    infoBox(
      "Updated data", paste(pc_updated(database(), tab_type_object)[1], "%"), icon = icon("fish"),
  ...
  output$box_current_nimble <- renderUI({
    ...
    infoBox(
      "Model ready for", pc_updated(database(), tab_type_object)[2], icon = icon("calendar-check"),
  ...
  output$TemplateDataCall <- downloadHandler(
    ...
    content = function(file) {
      ...
      slice_db <- subset(data_for_template,
  ...
  welcom_mess <- reactive({
    ...
    if(USER$login == FALSE){
      ...
      welcoming<-as.character(paste("Welcome onboard,",tab_users[which(tab_users$username==isolate(input$Biencuits)),"firstname"],
                                    tab_users[which(tab_users$username==isolate(input$Biencuits)),"lastname"], "!", sep=" "))
    }
    return(welcoming)
  }
  )
              ...
              tabPanel(
                ...
                column(
                  ...
                  column(
                    ...
                    hidden(
                      div(id='before_emerg',
                          h5("In some cases, and because all eventualities cannot be anticipated, the check procedure of
