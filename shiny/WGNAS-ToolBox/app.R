...
if (Sys.info()[["user"]] == "joliviero") {
  ...
  setwd("C:/workspace/DIASPARA_WP3_migdb/shiny/WGNAS-ToolBox/")
}
...
server <- function(input, output, session) {
  ...
  # drv <- dbDriver("PostgreSQL") CB > already launched in global.R
  ...
  tab <- reactive({
        ...
        # validate(
        #  need(check_input_data_set(data_to_update(),database()[,2:9], typecheck)=='data is valid', check_input_data_set(data_to_update(),database()[,2:9], typecheck))
        # )
        if (!is.null(data_to_update())) {
          if (check_input_data_set(data_to_update(), database()[, 2:9], typecheck) == "data is valid") {
            sloubi %>%
  ...
  observeEvent(
      ...
      {
        validate(
            need(check_input_data_set(data_to_update(), database()[, 2:9], typecheck) == "data is valid",
                check_input_data_set(data_to_update(), database()[, 2:9], typecheck))
        )
        reset_navig(next_trans() - previous_trans())
      }
  )
  ...
  observe({
        toggleState(id = "valid", condition = !input$Name %in% c("", "Enter text...") &
                ...
                check_input_data_set(data_to_update(), database()[, 2:9], typecheck) == "data is valid")
      })
  ...
  observeEvent(input$valid, {
        ...
        # Let's first assess if all the variables with a temporal component cover the same period. 
        #   If yes, generate_nimble will be set to 1.
        
        generate_nimble <- fun_check_time_cov_before_nimble(
            .tab_type_object = tab_type_object, 
            .databasenewval = databasenewval)
  ...
  observeEvent(input$activ_emerg, {
        if (input$activ_emerg == T) {
         typecheck <- "emergency_procedure"
        } else {
         typecheck <- "varying_SU"
        }
