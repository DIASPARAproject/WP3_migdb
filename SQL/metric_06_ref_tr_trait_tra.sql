...
CREATE TABLE refeel.tr_trait_tra (  
  CONSTRAINT fk_tra_wkg_code  FOREIGN KEY (tra_wkg_code)
  REFERENCES ref.tr_icworkinggroup_wkg(wkg_code)
  ON UPDATE CASCADE ON DELETE RESTRICT,
  CONSTRAINT fk_tra_spe_code  FOREIGN KEY (tra_spe_code)
...
COMMENT ON COLUMN refeel.tr_trait_tra.tra_id IS 'Integer, id of the trait';
COMMENT ON COLUMN refeel.tr_trait_tra.tra_code IS 'Name of the trait';
COMMENT ON COLUMN refeel.tr_trait_tra.tra_description IS 'Description of the fish trait';
COMMENT ON COLUMN refeel.tr_trait_tra.tra_typemetric IS 'Is the metric a group metric (group), or individual metric (individual) or can be used in both tables (both) ?';
COMMENT ON COLUMN refeel.tr_trait_tra.tra_qualitativeornumeric IS 'Indicate variable type, either Qualitative or Numeric';

GRANT ALL ON refeel.tr_trait_tra TO diaspara_admin;
GRANT SELECT ON refeel.tr_trait_tra TO diaspara_read; 